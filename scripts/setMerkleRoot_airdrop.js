...
async function main() {
  ...
  const nftContract = await nftFactory.attach(
    '0x8f6451715b549c30777331e552f53C9a7E5CB97C' // Deployed contract address, 0x7a3fbd8266b760e1e9cB2A5795942509C8fF42ae
  )

  // Re-calculate merkle root from the whitelist array.
  const leafNodes = allowlist_airdrop.map((addr) => keccak256(addr))
  const merkleTree = new MerkleTree(leafNodes, keccak256, { sortPairs: true })
  const root = merkleTree.getRoot()

  // Set the re-calculated merkle root to the contract.
  const res = await nftContract.setMerkleRoot(root, 1)
  console.log({ res })
